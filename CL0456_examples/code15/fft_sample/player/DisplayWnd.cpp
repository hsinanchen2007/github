...
BOOL CDisplayWnd::CreateCurveWindow(CWnd *pParent, const CRect& rect)
{
	ASSERT(GetSafeHwnd() == NULL);

    if(!CWnd::Create(NULL, _T("CurveWindow"), WS_CHILD|WS_VISIBLE, rect, pParent, 0, NULL))
    {
        return FALSE;
    }
...
static LPCTSTR lpszFx = _T("原函數曲線");
static LPCTSTR lpszS = _T("三次樣條函數曲線");
...
void CDisplayWnd::DrawCurve(CDC *pDC)
{
    ...
    {
        DrawSpline(pDC, m_xPpoint, m_yPpoint, RGB(0,255,255));
    }
