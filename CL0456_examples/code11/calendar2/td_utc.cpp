...
{ 
    ...
    if(y >= 2005)
    {
         //sd是2005年之後幾年（一值到y1年）的速度估計。
         //sjd是y1年之後的加速度估計。瑞士星曆表jsd=31,NASA網站
         int y1 = 2014;
         double sd = 0.4;
         int jsd = 31;
         if(y <= y1) 
             return 64.7 + (y - 2005) * sd; //直線外推
         double v = deltatExt(y, jsd);        //二次曲線外推
         double dv = deltatExt(y1, jsd) - ( 64.7 + (y1 - 2005) * sd ); //y1年的二次外推與直線外推的差
         if(y < y1 + 100) 
             v -= dv * (y1 + 100 - y) / 100;
         return v; 
    }
