...
int EditDistance(char *src, char *dest, int i, int j)
{
    ...
    else
    {
        ...
        {
            int edIns = EditDistance(src, dest, i, j + 1) + 1; //source 插入字元
            int edDel = EditDistance(src, dest, i+1, j) + 1; //source 刪除字元
            int edRep = EditDistance(src, dest, i+1, j+1) + 1; //source 替換字元
...
int EditDistance2(char *src, char *dest)
{
    ...
    int edIns = EditDistance2(src, dest + 1) + 1; //source 插入字元
    int edDel = EditDistance2(src + 1, dest) + 1; //source 刪除字元
    int edRep = EditDistance2(src + 1, dest + 1) + 1; //source 替換字元
...
int EditDistance3(char *src, char *dest)
{
    ...
    for(i = 1; i <= strlen(src); i++)
    {
        for(j = 1; j <= strlen(dest); j++)
        {
            ...
            {
                int edIns = d[i][j - 1] + 1; //source 插入字元
                int edDel = d[i - 1][j] + 1; //source 刪除字元
                int edRep = d[i - 1][j - 1] + 1; //source 替換字元
