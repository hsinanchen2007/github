...
void EdgeCenterMarkFill(const Polygon& py, int color)
{
    ...
    InitScanLineEdgesTable(et, py);//初始化邊表
    
    FillBackground(A - color); //對整個填滿區域背景顏色取補
    for_each(et.begin(), et.end(), EdgeScanMarkColor);//依次處理每一條邊
}
...
void EdgeFenceMarkFill(const Polygon& py, int fence, int color)
{
    ...
    for_each(et.begin(), et.end(), 
        std::bind2nd(std::ptr_fun(FenceScanMarkColor), fence));//依次處理每一條邊
}
