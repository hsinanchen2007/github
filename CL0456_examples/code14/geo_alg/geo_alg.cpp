...
bool IsPointOnLineSegment(const LineSeg& ls, const Point& pt)
{
    ...
    double cp = CrossProduct(ls.pe.x - ls.ps.x, ls.pe.y - ls.ps.y, 
                             pt.x - ls.ps.x, pt.y - ls.ps.y); //計算叉積

    return ( (IsPointInRect(rc, pt)) //排除實驗
...
{
    if(IsLineSegmentExclusive(ls1, ls2)) //排斥實驗
    {
        return false;
    }
    //( P1 - Q1 ) ℅ ( Q2 - Q1 )
...
{
    assert(py.IsValid()); /*只考慮正常的多邊形，邊數>=3*/

    int count = 0;
    LineSeg ll = LineSeg(pt, Point(-INFINITE, pt.y)); /*射線L*/
